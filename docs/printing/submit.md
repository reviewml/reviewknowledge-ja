...
たとえば基準枚数が 4 となっている場合、80 ページであれば 4 の倍数なので適切ですが、82 ページだと倍数でないため不適切です。
...
- 種類「TrueType」 というものが出ることもありますが、こちらも埋め込みになっていれば問題ないようです。
...
ブラウザから出力される PDF は RGB です。そのため、入稿仕様に CMYK と書かれていたら、CMYK にする必要があり、また白黒印刷用途であれば K 版のみ（グレースケール化）にしなければなりません。
