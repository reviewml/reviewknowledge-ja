...
この理由としては、Re:VIEW の内部実装をかなり変更したため、review-ext.rb を使って Re:VIEW の挙動を変更・拡張していたプロジェクトには影響が多大となる可能性があることによるものです。

逆に言えば、review-ext.rb を使っていない範囲であれば、バージョン 3 系のプロジェクトへの影響は軽微で、ほぼそのまま動くはずです。

本記事では、リリースノートをベースに、新規に導入した機能や変更点について理由を挙げながら解説します。
...
## review-ext.rb への対応について
今回の内部実装の変更に際しては、review_version パラメータを見て後方互換性をとるといったことはしていません。互換性保持のポリシーはあくまでも生成結果についてのことであり、内部実装の変化については今後とも一切の保証はありません。

なお、[IndexBuilderの導入](https://github.com/kmuto/review/pull/1384) を近い将来で検討しており、これによってまたやや大きめの内部変更が発生する見込みです。
