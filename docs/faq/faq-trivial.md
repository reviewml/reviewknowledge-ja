...
Ruby 言語で書かれています。2020年10月時点では、Ruby 2.4 以上を想定しています。
...
これに対し、タイプセット環境でどのように表現するかは紙面デザインによってアプローチが異なり、万能の形状はありません。あるデザインに特化してしまった変換物では、ほかのデザインでまったく不都合になることがままあるため、Re:VIEW が出力する変換物は極力シンプルで、後で加工することを想定したものです。Re:VIEW が生成した変換物をどのようにタイプセット環境での表現と合致させ、加工するかは、タイプセット環境についての知識が**必須**です。
